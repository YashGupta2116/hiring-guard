...
veritrust-ml/
  ...
  src/vtml/
    ...
    evaluate/                    # Phase 4: two synthetic fixtures + priors,
      ...
      figures.py                 # F1-F5, styled per Design.md; TOKENS is the
                                  # only other place besides vtml.mplstyle
                                  # allowed to name a colour
      report.py                  # writes reports/eval-<version>.md
      priors_sweep.py            # out-of-phase: F5, sweeps each scored
                                  # channel's prior 0.25x-4x against both
                                  # fixtures, see Memory.md fairness-of-priors
      calibration_ablation.py    # out-of-phase: replays the pre-fix
                                  # calibration-window defect via a local
                                  # subclass, never imported elsewhere
  ...
  tests/                         # 101 passing (89 through Phase 6/contract, +12
                                  # from priors_sweep/calibration_ablation, see
                                  # Memory.md)
    test_types.py
    test_fusion_math.py
    ...
    test_replay_golden.py        # Not built. Tagged Phase 4 here historically,
                                  ...
                                  # actually uses
    test_contract.py             # Guards ../contracts/detector-registry.json
                                  # against drift from Channel/REGISTRY -- the
                                  # backend reads the same file, see
                                  # ../../docs/cross-component-architecture.md
    test_priors_sweep.py         # out-of-phase: no-leakage, determinism,
                                  # audio exclusion, focus narrowest-channel
    test_calibration_ablation.py # out-of-phase: pins condition A/B scores
                                  # and in-window observation counts exactly
```
...
**Unbuilt, like section 1's diagram.** Phase 5 is cut and `handler.py` does not exist (see the banner
at the top of section 1). Nothing below runs anywhere; kept as the design this was scoped to reach.
...
**The DynamoDB and Lambda-specific rows below describe the same unbuilt runtime as sections 1 and 8.**
The engine-level behaviour (malformed observation, baseline fallback, latency budget) is real and
tested regardless of where it runs.
